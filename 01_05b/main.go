...
func matchSales(budget float64, items []SaleItem) []SaleItem {
	...
	for _, item := range items {
		if item.ReducedPrice <= budget {
			item.SalePercentage = (1 - item.ReducedPrice/item.OriginalPrice) * 100
			saleItems = append(saleItems, item)
		}
	}

	sort.Slice(saleItems, func(i, j int) bool {
		return saleItems[i].SalePercentage > saleItems[j].SalePercentage
	})
...
// importData reads the item entries from file and
